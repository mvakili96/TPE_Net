...
class RailSem19_C20_Loader(data.Dataset):
    #==================================================================================================================
    ...
    def __getitem__(self, index):
        ...
        labelmap0 = np.array(labelmap0, dtype=np.uint8)
            # completed to set
            #   labelmap0: ndarray (360, 480), uint8, val 0~19, 255
        ...
        if self.b_resize is True:
            ...
            labelmap_rsz = np.array( Image.fromarray(labelmap0).resize((w_rsz, h_rsz)), dtype=np.uint8)
                ...
                # labelmap_rsz: (h,w),   uint8, val 0~19, 255
        ...
        ###------------------------------------------------------------------------------------------
        # note that labels other than 0~19 should be 250 (which indicates invalid)

        set_idx_invalid = (labelmap_b > 19)
            # completed to set
        ...
        if self.augmentations is not None:
            ...
            #   labelmap_b: ndarray (360, 480), val 0~19, 250
        ...
        if self.b_do_transform:
            ...
            #   labelmap_b: torch.Size([360,480]), val 0~19, 250
    ...
    def decode_segmap(self, labelmap, plot=False):
        ...
        rgb_class19 = [  0, 255,   0]   # 19: centerline            <added by jungwon>
    ...
    #==================================================================================================================
    def decode_segmap_b(self, labelmap, plot=False):
        ###===================================================================================================
        ### convert label_map into visible_img [only called in __main__()]
        ###===================================================================================================
        ...
        ### create default img
        r = np.zeros_like(labelmap)          # 250: indicating invalid label
        g = np.zeros_like(labelmap)
        b = np.zeros_like(labelmap)


        ### find (centerline pixels)
        idx_set = (labelmap == 19)              # idx_set: ndarray, (h, w), bool

        ### assign
        r[idx_set] = 255       # r: 0 ~ 255
        g[idx_set] = 255       # g: 0 ~ 255
        b[idx_set] = 255       # b: 0 ~ 255


        img_label_rgb = np.zeros((labelmap.shape[0], labelmap.shape[1], 3), dtype=np.uint8)
        img_label_rgb[:, :, 0] = r
        img_label_rgb[:, :, 1] = g
        img_label_rgb[:, :, 2] = b
...
if __name__ == "__main__":
    ...
    path_root = "/home/yu1/Desktop/temp_C20/railsem19_C20"
        # completed to set
        #       path_root
        #---------------------------------------------------------------------------------------------
    ...
    #trainloader_head = RailSem19_C20_Loader(path_root, b_do_transform=True, augmentations=augmentations)
    trainloader_head = RailSem19_C20_Loader(path_root, b_do_transform=True)
        # completed to create
        #       trainloader_head
    ...
    for idx_this, data_samples in enumerate(trainloader):
        ...
        if 1:
            ...
            for j in range(batch_size):
                ...
                ### labelmap
                labelmap_np  = labelmaps.numpy()[j]                             # labelmap_np: ndarray (360, 480), val 0~19, 250
